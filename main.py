...
def student_menu(system, storage, student):
    title(f"STUDENT PORTAL — {student.name}")
    while True:
        print("\n>> Select an option: ")
        ...
        if choice == "1":
            ...
            professor = input("Professor name (blank for all, e.g. Sarath): ").strip().lower()
...
def faculty_menu(system, storage, faculty):
    title(f"FACULTY PORTAL — {faculty.name}")
    while True:
        print("\n>> Select an option: ")
...
def administrator_menu(system, storage, admin):
    title(f"ADMINISTRATOR PORTAL — {admin.name}")
    while True:
        print("\n>> Select an option: ")
...
def seed(system, storage):
    if not system.get_user("S001"):
        system.add_user(UserType.STUDENT, "S001", "Ravindu Dilupa", "ravibndu@ucsc.edu", major="Computer Science")
    if not system.get_user("F001"):
        system.add_user(UserType.FACULTY, "F001", "Dr. Sarath Hewage", "cs@ucsc.edu", department="Computer Science")
    if not system.get_user("F002"):
        system.add_user(UserType.FACULTY, "F002", "Dr. Rohan Gamage", "kw@ucsc.edu", department="Mathematics")
    if not system.get_user("F003"):
        system.add_user(UserType.FACULTY, "F003", "Dr. Eran Ranasinghe", "rp@ucsc.edu", department="Business")
    if not system.get_user("A001"):
        system.add_user(UserType.ADMINISTRATOR, "A001", "System Admin", "admin@ucsc.edu", privilege_level=3)
